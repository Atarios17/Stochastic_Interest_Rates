...
mc_bond_prices = [bond_price_mc(time_vec[:int(T/dt)], r_sims[:,:int(T/dt)]) for
...
our_bp = bond_option_price_HW1F(strike,option_mat,bond_tenor,hw_pchip.yield_curve,hw_pchip.alpha, hw_pchip.sigma, float(hw_pchip.yield_curve(0)))
print("Our Bond Option Price:")
print(our_bp)
t_vec, r_t = hw_pchip.simulate_rate_paths(T = option_mat, N_paths = 10000, dt = 0.0025)
mc_bp = bond_option_price_mc(t_vec, r_t, option_mat, bond_tenor, strike, hw_pchip.yield_curve, hw_pchip.alpha, hw_pchip.sigma)
...
bond_tenors = np.arange(0.25, 20 + 0.25, 0.25)
Our_Bond_Option_Prices = bond_option_price_HW1F(strike, option_mat, bond_tenors, hw_pchip.yield_curve, hw_pchip.alpha, hw_pchip.sigma)
MC_Bond_Option_Prices = np.array([bond_option_price_mc(time_vec[:int(option_mat/dt)], r_sims[:,:int(option_mat/dt)], option_mat, bond_tenor, strike, hw_pchip.yield_curve, hw_pchip.alpha, hw_pchip.sigma) for bond_tenor in bond_tenors])
...
plt.plot(bond_tenors,[ql_hw_model.discountBondOption(call_put, strike, option_mat, option_mat + T)
                      for T in bond_tenors], label = 'QuantLib', color = 'orange')
...
option_maturity = 15
opt_maturities = np.array(np.arange(0.25, option_maturity + 0.25, 0.25))

Our_Bond_Option_Prices = bond_option_price_HW1F(strike, opt_maturities, bond_tenor, hw_pchip.yield_curve, hw_pchip.alpha, hw_pchip.sigma)
MC_Bond_Option_Prices = np.array([bond_option_price_mc(time_vec[:int(opt_mat/dt)], r_sims[:,:int(opt_mat/dt)], opt_mat,
                                                       bond_tenor, strike, hw_pchip.yield_curve, hw_pchip.alpha, hw_pchip.sigma) for opt_mat in opt_maturities])
...
plt.plot(opt_maturities,[ql_hw_model.discountBondOption(call_put,strike,option_mat,option_mat+bond_tenor) for option_mat in opt_maturities], label = 'QuantLib', color = 'orange')
...
our_sp = jamshidian_swaption_price(hw_pchip.yield_curve, option_maturity, swap_tenor, fixed_rate, frequency, notional, hw_pchip.alpha, hw_pchip.sigma, True)
...
mc_sp = swaption_price_mc(time_vec[:int(option_maturity/dt)], r_sims[:,:int(option_maturity/dt)], option_mat = option_maturity, swap_tenor = swap_tenor, swap_freq = frequency, strike_fixed_rate = fixed_rate,
...
Our_Swaption_Prices = [jamshidian_swaption_price(hw_pchip.yield_curve, option_maturity, swap_tenor, fixed_rate, frequency, notional, hw_pchip.alpha, hw_pchip.sigma, True) for swap_tenor in swap_tenors]

MC_Swaption_Prices = np.array([swaption_price_mc(time_vec[:int(option_maturity/dt)], r_sims[:,:int(option_maturity/dt)], option_mat = option_maturity, swap_tenor = swap_tenor, swap_freq = frequency, strike_fixed_rate = fixed_rate,
                  yield_curve = hw_pchip.yield_curve, alpha = hw_pchip.alpha, sigma = hw_pchip.sigma, is_payer=True) for swap_tenor in swap_tenors])
...
plt.plot(swap_tenors,MC_Swaption_Prices, label = 'Monte Carlo', color = 'green')
plt.plot(swap_tenors,ql_swaption_prices, label = 'QuantLib', color = 'orange')
...
Our_Swaption_Prices = [jamshidian_swaption_price(hw_pchip.yield_curve, option_maturity, swap_tenor, fixed_rate, frequency, notional, hw_pchip.alpha, hw_pchip.sigma, True) for option_maturity in option_maturities]
